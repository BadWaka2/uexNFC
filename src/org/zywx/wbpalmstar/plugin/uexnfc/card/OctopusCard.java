...
import org.zywx.wbpalmstar.plugin.uexnfc.tech.FeliCa;
...
final class OctopusCard {
	...
	private static String parseName(int system, Resources res) {
		if (system == SYS_OCTOPUS)
			return EUExUtil.getString("plugin_uexnfc_name_octopuscard");

		if (system == SYS_SZT)
			return EUExUtil.getString("plugin_uexnfc_name_szt_f");
	...
	private static String parseInfo(FeliCa.Tag tag, Resources res) {
		...
		final String p = EUExUtil.getString("plugin_uexnfc_lab_pmm");
		r.append("<b>").append(i).append("</b> ").append(tag.getIDm().toString());
		r.append("<br />").append(p).append(' ').append(tag.getPMm().toString());
	...
	private static String parseBalance(float[] value, int count, Resources res) {
		...
		final String c = EUExUtil.getString("plugin_uexnfc_lab_cur_hkd");
