...
final class VicinityCard {
	...
	private static String parseName(int type, Resources res) {
		if ((type & SYS_SZLIB) == SYS_SZLIB) {
			...
			if ((type & DEP_SZLIB_CENTER) == DEP_SZLIB_CENTER)
				dep = EUExUtil.getString("plugin_uexnfc_name_szlib_center");
			else if ((type & DEP_SZLIB_NANSHAN) == DEP_SZLIB_NANSHAN)
				dep = EUExUtil.getString("plugin_uexnfc_name_szlib_nanshan");
			else
				dep = null;

			final String srv;
			if ((type & SRV_BOOK) == SRV_BOOK)
				srv = EUExUtil.getString("plugin_uexnfc_name_lib_booktag");
			else if ((type & SRV_USER) == SRV_USER)
				srv = EUExUtil.getString("plugin_uexnfc_name_lib_readercard");
			else
				srv = null;
		...
		return EUExUtil.getString("plugin_uexnfc_name_unknowntag");
	}

	private static String parseInfo(byte[] id, Resources res) {
		final StringBuilder r = new StringBuilder();
		final String i = EUExUtil.getString("plugin_uexnfc_lab_id");
	...
	private static String parseSzlibData(int type, byte[] raw, byte[] sta, int blkSize, Resources res) {
		...
		if ((type & SRV_USER) == SRV_USER)
			sid = EUExUtil.getString("plugin_uexnfc_lab_user_id");
		else
			sid = EUExUtil.getString("plugin_uexnfc_lab_bktg_sn");
		...
		if ((type & SRV_BOOK) == SRV_BOOK) {
			...
			if ((type & DEP_SZLIB_NANSHAN) == DEP_SZLIB_NANSHAN) {
				if (cat == 0x10)
					scat = EUExUtil.getString("plugin_uexnfc_name_bkcat_soc");
				else if (cat == 0x20) {
					if (raw[11] == (byte) 0x84)
						scat = EUExUtil.getString("plugin_uexnfc_name_bkcat_ltr");
					else
						scat = EUExUtil.getString("plugin_uexnfc_name_bkcat_sci");
				} else
					scat = null;
			} else {
				scat = null;
			...
			if (scat != null) {
				final String scl = EUExUtil.getString("plugin_uexnfc_lab_bkcat");
